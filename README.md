...
Bug Triage Loop
Proposal Response Assembler
Slack Thread → Decision Brief
Design Review Loop
...
An allow/deny runbook (see Shared guardrails) that every loop respects
...
Pass --dry-run to see the proposed output without writing or posting anything. Pass --approve auto only in CI environments where every output has already been reviewed upstream.
...
Inspect — reads README, package.json/pyproject.toml, directory tree, key source files
Draft — produces a structured brief (see template below)
Self-check — Claude re-reads the repo against its draft and flags anything missing or wrong
Human gate — shows diff, waits for y/n
Write — commits ONBOARDING.md if approved
Output template:
...
Ingest — parses bug reports (JSON or plain text)
Score — severity (P0–P3), reproducibility (confirmed/likely/unclear), affected surface
Draft — triage comment per bug + grouping suggestions for duplicates
Self-check — Claude asks itself: "Would a senior engineer be satisfied with this triage?" and flags any P0 labelled without a reproduction path
Human gate — table view of all proposed labels/comments, approve individually or in bulk
Post — writes to GitHub Issues (or appends to triage-output.md in dry-run mode)
...
Extract requirements — numbered list of every stated requirement and evaluation criterion
Draft response — one section per requirement, with evidence from your company's past work (provide as context or it will note gaps)
Compliance matrix — table mapping every RFP requirement → response section → confidence level
Self-check — Claude reads both documents and calls out any requirement with no clear response
Human gate — diff view + compliance matrix; human edits before finalising
Output — proposal-response.md + compliance-matrix.csv
...
Sanitise — Python script (loops/slack-to-brief/sanitize.py) strips PII before Claude ever sees the data
Extract — Claude reads the sanitised thread and extracts: decision, context, alternatives, dissent, next steps, owner, due date
Draft brief — structured 1-page document
Self-check — Claude asks: "Is there anything in this brief that could re-identify a participant?" and flags it
Human gate — shows brief + a diff of what was redacted; human confirms before sharing
...
Ingest — reads design spec / Figma JSON export
Evaluate — scores each checklist item: Pass / Needs Work / Fail
Draft findings — for each Fail or Needs Work, a specific, actionable suggestion
Self-check — Claude asks: "Is every suggestion specific enough that a designer could act on it without a follow-up conversation?"
Human gate — rendered checklist + suggestions; reviewer can promote, demote, or edit before posting
Output — design-review.md or Figma comment payload
...
Read — parses source file, extracts functions and their signatures
Generate — writes an initial test file covering happy path, edge cases, and known error conditions
Run — executes the test suite and captures output
Self-check / revise — Claude reads failures, diagnoses root cause (bad test vs. real bug), and either fixes the test or flags a suspected bug to the human
Iterate — repeats up to 5 times (configurable via --max-iterations)
Human gate — final test file shown for approval before writing to disk
Output — utils.test.js (or equivalent)
...
Never pass raw PII to the API. Run the sanitiser first; Claude only ever sees the sanitised version.
API key in env only. No hardcoded keys; the .env file is in .gitignore.
Audit log. Every loop writes an append-only audit.log with timestamp, loop name, input hash, and what action was approved or denied.
Human approval flow
...
Publishing to Agent Commons
Agent Commons is a shared registry of tested, reusable agent patterns. To publish a loop from this repo:

Verify the eval rubric — every criterion must score ≥ 3
...
Contributing
Fork the repo
Add your loop under loops/<your-loop-name>/
Include sample data in sample-data/<your-loop-name>/
Add at least one eval in evals/<your-loop-name>/
Ensure the shared guardrails pass: python scripts/check_guardrails.py <your-loop-name>
Open a PR — the CI pipeline runs the reproducibility check automatically
